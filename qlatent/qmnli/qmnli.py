...
class QMNLI(QABSTRACT):
    ...
    def run(self, model=None, pre_text=None):
        # pre_text is accepted for compatibility with the QDELEGATOR chain
        # (QFILTER/QCACHE pass it positionally). MNLI has no place to use it,
        # so it is ignored.
        super().run(model)
        ...
        ## Apply softmax on logits (HF pipeline postprocess -> float scores)
        ...
        ## _pdf["P"] is the pandas view (floats), _t is the tensor view that
        ## qabstract_torch's math methods (mean_score, effect_size, softmax,
        ## minmax) read from. Same numbers, no grad tracked.
        self._pdf["P"] = p
...
class _QMNLI(QMNLI):
    ...
    def report(self, scale: Union[str, int] = "intensifier", index="emotion",
               filters: Dict[str, FILTER] = {"unfiltered": {}},
    ...
    def to_dataframe(self, scale: Union[str, int] = "intensifier",
                     index: List[str] = "emotion",
                     filter: FILTER = {},
