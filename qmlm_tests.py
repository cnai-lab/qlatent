...
mlm = pipeline("fill-mask",device=device, model="bert-base-uncased")
mlm.model_identifier = "bert-base-uncased"
...
asiq2_template = '{fraction} {gender} are asking for gender equality. {frequency} to get {objective}'
...
class ASIQ2(QMLM): # qmnli only
    def __init__(self, **kwargs):
        super().__init__(
            template="{fraction} {gender} are asking for gender equality. {frequency} to get {objective}",
...
#                 template="Over the last 2 weeks, I have been {emotion} about different things... I {intensifier} feel that way.",
#                 dimensions = {"emotion" : {'worrying' : 1, 'stressing' : 1, 'untroubled' : -1, 'confident' : -1},
...
class TestQmnli1(unittest.TestCase):
    def test_regular_fields(self):
        ...
        self.assertEqual(q._template, asiq2_template)
...
class TestQmnliFilterDataFrame(unittest.TestCase):
    def test_filter_data_frame(self):
        ...
        df = qmlm._filter_data_frame(df, filter) #filter_data_frame is found in qabstract
...
# class EXAMPLE1(QMLM): #qmnli only
#     def __init__(self, **kwargs):
#             super().__init__(
#     #             context="Over the last 2 weeks...",
#     #             template="I {intensifier} felt {emotion}.",
...
class EXAMPLE1(QMLM):
    def __init__(self, **kwargs):
        super().__init__(
        template="{fraction} {gender} are asking for gender equality always to get {objective}",
